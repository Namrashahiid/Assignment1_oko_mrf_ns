...
Here are we filtering the data, with specified country, which is Spain (ES), in year 2010.
...
One important thing here to point out is that the reason behind that the values are equal to zero can be because of maybe there are some data/values that are lost or nor there for this specific year which we have chosen.
