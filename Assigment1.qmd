...
In this assignment are we going to study the regional inequality of our selected Europe countries.
...
The main in this assignment is going to be calculation of GDP per capita and our dataset from Eurostat.
...
We find different data in the columns of the dataset "Nama_10r_3gdp.csv".
This dataset contains data on Nuts 3 level.

We are also using dataset: "Demo_r\_pjanaggr3.csv".
This dataset contains information for population data on Nuts 3 level.
...
Like mentioned in this assignment we will use GDP per capita, which is based on population by NUTS.
...
Now the question is how is GDP per capita calculated?
...
Main function of metadata is to explain data by giving definition, for example according to our data, region, year, GDP, population and GDP per capita.
...
When we look at this table what we can see is: decriptive statistics of data, we see data of different regions.

In this summay statistics table we can see different type of data, which is first of all: variables such as: year, population (pop), GDP per capita (GDP_capita).
After that we find N and Mean.
When we look at the difference between the statistics such as Max and Min, and if we compare both variables, there is no such major difference between the values.
...
If we look at the other countries, for example Poland.
We can see that, it does also had an effect of financial crisis in years 2008-2009 like Norway had.
...
demo <- read_csv('demo_r_pjangroup.csv') %>%
  filter(TIME_PERIOD == "2010" & unit == "NR" & age == "TOTAL" & sex == "T" & (grepl('IE..', geo) | grepl('PL..', geo) | grepl('ES..', geo) | grepl('SI..', geo) | grepl('NL..', geo) | grepl('NO..', geo) | grepl('AL..', geo))) %>%
...
data2010 <- gdp_pc_n2 %>%
  ...
  left_join(gdp_pc_n2, by = c("year", "id_nuts2")) %>%
  # level of economic development = log(gdp per capita)
  mutate(log_gdppc = log(gdp_pc_n2)) %>% #....Det e denna som ikkje funka....
...
lm.1 = lm(gini ~ log_gdppc, data = IE2010)
lm.2 = lm(gini ~ log_gdppc, data = PL2010)
lm.3 = lm(gini ~ log_gdppc, data = ES2010)
lm.4 = lm(gini ~ log_gdppc, data = SI2010)
lm.5 = lm(gini ~ log_gdppc, data = NL2010)
lm.6 = lm(gini ~ log_gdppc, data = EL2010)
lm.7 = lm(gini ~ log_gdppc, data = NO2010)
lm.8 = lm(gini ~ log_gdppc, data = AL2010)
lm.9 = lm(gini ~ log_gdppc, data = data2010)
...
ggplot(aes(x = eduval, y = gini, fill = nuts_id2, color = nuts_id2)) +
  geom_point(lwd = .8) +
   labs(x = "Edu", y = "Gini")
```
