...
Looking at this table, we can see decriptive statistics of data different regions.

In this summay statistics table, we can see different type of data, which are primarily variables such as: year, population (pop), GDP per capita (GDP_capita).
After that we find N and mean.
N stands for how many regions are in the dataset.
When we look at the difference between the statistics, such as max and min, and if we compare both variables, there is not much difference between the values.
