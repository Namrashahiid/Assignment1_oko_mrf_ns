...
Therefore our focus here is countries which are showing
up, in this case: Slovenia (SI), Poland (PL), Netherlands (NL) and Spain (ES).

We are here looking at R2, which is value for coefficient of determination.
This R2 coefficient of determination can be defined as a regression problem metric, that help the performance or the result of the model in R to be better.
...
What we can analyze here by
looking at this value R2 1.000 for Slovenia (SI), is that the value that is analyzed is suitable or fitted for each other.
On the other hand when we look at the R2 value for Spain (ES), we can see that it
is 0.0.
...
What we can see from this graph is a estimate.
The parameters we are looking at here is: Log GDP per capita on the x-axis and Gini nuts 2 values at the y-axis.
...
We can see that the line on this graph is negative, this indicates that one of values on regression model
formel, is negative.
...
Graph\* shows us the estimation plotted for Spain (ES).
...
Below we will look at how Spain (ES), Netherlands (NL) and Poland (PL) fit the assumptions of OSL.
