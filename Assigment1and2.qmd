...
What wee see here for Poland is: there is 10 numbers for this country and the coefficient of determination R2 is 0.398

What we see here for Spain is: there is there is 11 numbers and the coefficient of determination R2 is 0.
Value 0 can be due to

As mentioned the value 0 can be due to low
...
The question now is, what is regression analysis?
...
tmp_n2 %>%
  filter(year == 2010) %>%
  select(gdp_capita_n2, id_nuts2, gini_n2) %>%
  # filter(id_nuts2 %in% tmp_gdp_gini$id_nuts2) %>%
  ggplot(aes(x = gdp_capita_n2, y = gini_n2)) +
  geom_point()
```

### Discuss the goodness of fit of your estimated model:

What we have done here is that we are analyzing our model.
...
tmp_n2 %>%
  ...
  select(gdp_capita_n2, id_nuts2, gini_n2) %>%
  # filter(id_nuts2 %in% tmp_gdp_gini$id_nuts2) %>%
  ggplot(aes(x = log(gdp_capita_n2), y = gini_n2)) +
  geom_point()+
...
Here from this table\* the β\^1 value is ca: 0.12 and β\^2 value is ca: 5800
...
```{r}
#Usikker på om vi skal ha denne?
tmp_gdp_gini <- tmp %>% 
...
From this table we can see the data set for regions Spain ES41 which is Castilla Y Le has the highest Gini nuts 2.
What we fin under Gini nuts 2 is the data for income, consumption and standard of living.
