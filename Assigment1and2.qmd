...
What we are doing here is that, we are regenerating data for GDP per capita from the last assignment.
...
What we can see here from this table is GDP_capita mapping.
In this part are we using the fundamentals of mapping, we are using ggplot.

```{r}

a6cb429d1add71857695fceb310a856095b4cc74
tmp_gdp_gini <- tmp %>% 
  filter(year == 2010, ) %>%  
