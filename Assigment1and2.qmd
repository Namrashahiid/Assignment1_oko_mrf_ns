...
In last assignment (Assignment 1) we focused on GDP per capita and different types of graphs, Gini and tables

In this assignment we are going to have our main focus on countries we have focused on in the first assignment.
In the first part of this assignment we will use the same data that were presented in the last assignment.

So the first task in this assignment is to estimate the effect of regional development yj on regional inequality GINIWj for the year 2010.To study Growth and inequity, it is essential for us to look at and focus on GDP per capita.
...
Important fact to come up here is that when we have higher value of GDP per capita, then it is most probably because of the higher incomes and consumption.
...
On the other side, when the value is equal to 1 or close enough to 1, then this indicates that the model is most probably suitable or fit to analyze.
...
A log in R wil transform our data in a way that will apply a logarithm to our data that will help reduce its skew.
...
As we have mentioned earlier, the gini value is from 0 and 1 in the table.

The reason behind the values 0, can also be because of that maybe one of the values are missing.
Therefor this leads us to gini values 0.
...
One important thing here to point out is that the reason behind that the values are equal to zero can be because of maybe there are some data/values that are lost or not there for this specific year which we have chosen.What we have done in this formel up, is that we have regenerated the data, so that we can get the right values for our next steps.
...
The reason behind this most probably, we are missing some observation in our ...dataset
...
What wee see here for Poland (PL) is that there is 10 numbers for this country and the coefficient of determination R2 is 0.398.
...
We can see by looking at this method what Y value would be when X = 0.
...
The countries are: Slovenia, Spain, Netherlands and Poland.
If we plus the

What we can see here in this model is GDP per capita on the x-axis (nuts 2 level) and gini (nuts 2) on y-axis.
The fact of the matter is that the coefficient value relies between 0 and 1.
On the y-axis the values on the graph relies between 0 and 0,20.
Observations that are the black dots on the graph are spread all over.
...
In this part are we going to analyze estimation for 3 countries.
Countries which we are going to use as a estimation for our plots are: Spain, Poland and Netherlands.

We have now worked with simple regression model previously.
Coefficients of resolution is important for put next steps in this assignment.
So the first country, which we are going to look at is Spain.
...
The table shows us that when prosperity increases, income and GDP per capita are less different.
...
What we can see from this table is a estimate.
The parameters we are looking at here is: GDP per capita on the x-axis and Gini nuts 2 values at the y-axis.
If we call these parameters with econometric language, we call them β1 and β2 values.
We call these values prediction of the depended variable, fitted values.

We see that together these values gives us a trace line: Gini nuts 2i = β\^1 + β\^2GDP per capitai.

Here from this table\* the β\^1 value is ca: 0.12 and β\^2 value is ca: 11.0
...
Graph\* shows us the estimation plotted in this graph for Spain, we can see that it lies in the middle and is more flat like a horizontal line on the graph.
...
For example the region in Poland PL52 Polnocho-Zachodni has high GDP per capita, and at the same time high Gini.
...
We can see different type of spread data all over the graph.
NL11 region i Netherlands East Groningen has the highest GDP per capita and Gini according to this Graph.

A can be defined as the difference between the values, which we observe and analyze.
These values are linked to the regression model.
...
Below we will look at how Spain, Netherland and Poland fit the assumptions of OSL........

```{r}
#| label: fig-par-plot
#| fig-cap: Diagnostic plots
mylm = lm_function(ES2010)
par(mfrow = c(2,2))
plot(mylm)
```
...
#| label: fig-par-plot
#| fig-cap: Diagnostic plots
mylm = lm_function(PL2010)
par(mfrow = c(2,2))
plot(mylm)
```

Breusch-Pagan Test can be discussed here also.
...
"Downloaded data from Eurostat for you subset of countries regional (NUTS2, j ) data related to transport infrastructure, education and demographics. Select on variable per category that you would like to explore further in there relationship to regional inequality GINIWj."
...
edu <- read_csv('edat_lfse_04.csv') %>%
  filter(TIME_PERIOD == "2010", age == "Y25-64" & grepl('IE..', geo) | grepl('PL..', geo) | grepl('SI..', geo) | grepl('NL..', geo) | grepl('ES..', geo) | grepl('NO..', geo) | grepl('AL..', geo)) %>%
  select(unit, geo, age, sex, TIME_PERIOD, OBS_VALUE) %>%
...
ggplot(aes(x = edu1, y = gini_n2 , fill = id_nuts2, color = id_nuts2)) +
  geom_point(lwd = .8) +
...
#Gen Table
  ...
  statistics = c(N = "nobs", R2 = "r.squared"),
  coefs = c(education = "edu1","infra1", life.exp. = "demo1", const. = "(Intercept)"),
  note = "Note: {stars} T statistics in brackets."
  )
}
```
