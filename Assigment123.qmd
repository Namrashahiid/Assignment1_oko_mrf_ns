...
Looking at @wealthiest, @tab-least_wealthiest and @tbl-summary_gdp over, we can see substantial differences in region size as well as economic development.
...
[Skal me ha det? Såg han hadde det på html eksempelet på canvas.]{.underline}
...
[To study Growth and inequity, it is essential for us to look at and focus on GDP per capita. GDP per capita is a indicator of economic performance and is very normal to use to analyse standards of living and for economical perspective. Important fact to come up here is that, although GDP per capita is often used as a broad measure of average living standards, but high GDP per capita not necessarily mean high level of households disposable income, a key measure of average material capita in the OECD. (sjekk for eventuell gjentakting frå ass 1)]{.underline}
...
From @tbl-stastics_log_gdppc we see for Poland (PL) that there is 10 observations and the coefficient of determination R2 is 0.398.
What we see here for Spain (ES) is that there is there is 11 obersvations and the coefficient of determination R2 is 0.
...
#| fig-cap:  line
ggplot(data = data2010, aes(x = gdp_pc_n2, y = gini_n2)) +
  geom_point() +
...
What our aim here to do is that we use our entire datast to estimate the effect of regional developmennt yi on regional inequality GINIWj in a penal setup.
...
The std- error value is on 12153.32.
...
The std- error value is on 16700.21.
We can see that the std-error value is pretty high, which tells us that the values are more spread out and are around the mean in a dataset.
...
```{r}
#table(tbl-summary_gdp)
```

```{r}
#table(tab-least_wealthiest)
```
