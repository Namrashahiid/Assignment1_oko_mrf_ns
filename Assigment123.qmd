...
## 4.1 Effect of development for different subset:
...
Our next task is now to explore alternative functional forms of our previous estimates on the relationship between regional development yi on regional inequality GINIWj.
...
If we look at the interaction (intercept) value, which is 0.534, we can say here that it is pretty high, which can indicate that there in a interaction between log, log\^2 and log\^3 in all countries.
...
When we look at the R2 value on the interaction, we can here conclude by saying that, this R2 value 0.221 is small, and the R2 is relatively not strong, in other words weak.
...
The conclusion here is going to be that the given data on this table fits more than the previous one, because the R2 value under interaction on previous table was 0.161 and on this table the R2 value under interaction is 0.221, which is higher than the previous one.
...
The p-value is: 0.6586212.
Our p-value is pretty high on as mentioned 0.6586212.
We can conclude here by saying that, we are not able to reject the H0 hypothesis.
...
Our p-value is pretty high on as mentioned 0.8285633.
We can conclude here by saying that, we are not able to reject the H0 hypothesis.
Hence we are not facing Heteroskedasticity.
