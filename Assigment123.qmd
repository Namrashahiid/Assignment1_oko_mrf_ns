...
In @tbl-mlr-all_counties, if we look at the interaction (intercept) value, which is 0.534, we can say here that it is pretty high, which can indicate that there in a interaction between log, $log^2$ and $log^3$ in all countries.
...
What we can say out from @fig-gdp_pc_nuts0, is that Albania has a low GDP per capita, in comparison to other countries.
This can be the reason why EU, wants to help and make sure that the country is doing well financially.
For Albania it can be a eventual problem to get help from EU.
