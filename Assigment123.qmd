...
[A log in R wil transform our data in a way that will apply a logarithm to our data that will help reduce its skew. In other words it will reproduce or rescale our data more in a statistical way. **(Meina han sa me va på rett veg, men skrive om slik at det gir meining)**]{.underline}
...
lm.4 <- (gini_n2 * 100 ~ gdp_pc_n2, data = filter(data2010, year == 2010)) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.5 <- (gini_n2 * 100 ~ gdp_pc_n2, data = filter(data2010, year == 2010)) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.6 <- (gini_n2 * 100 ~ gdp_pc_n2, data = filter(data2010, year == 2010)) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.7 <- (gini_n2 * 100 ~ gdp_pc_n2, data = filter(data2010, year == 2010)) %>%
  coeftest(vcov = hccm, save = TRUE)
