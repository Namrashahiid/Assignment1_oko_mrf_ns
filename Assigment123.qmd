...
-   ***Kun grafen oppe til venstre (residuals / fitted values) som er viktig for oss***
...
The file 'demo_r\_pjangroup.csv' shows us the population on 1 January by age group, sex and NUTS 2 region.
Next the file 'edat_lfse_04.csv' shows the population by educational attainment level, sex and NUTS 2 regions (%).
Finally the file 'tran_r\_vehst.csv' shows us stock and vehicles by category and NUTS2 regions.
...
library(afex)
df$id = 1:nrow(df)
aov_ez(data=df, id="id", between="gdp_pc_n2", dv"values"
```
...
lm.3 <-(gini_n2 * 100 ~ gdp_pc_n2, data = filter(data_n2, year == 2010)) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.4 <-(gini_n2 * 100 ~ gdp_pc_n2, data = filter(data_n2, year == 2010)) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.5 <-(gini_n2 * 100 ~ gdp_pc_n2, data = filter(data_n2, year == 2010)) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.6 <-(gini_n2 * 100 ~ gdp_pc_n2, data = filter(data_n2, year == 2010)) %>%
  coeftest(vcov = hccm, save = TRUE)
