...
[Skrive noko om gini Polen her @fig-gini_poland]{.underline}
...
[Skal me ha med eller uten log?]{.underline}
...
[Skrive om @fig-lm-res_pred, grafen over.]{.underline}

What we can see from this graph @fig-lm-data2010 is a ascending linear regression.
For example the region in Netherlands NL32 North Holland has high GDP per capita, and at the same time high Gini value.

What we see here from this graph @fig-lm-data2010 is a regression analysis for Netherlands.
...
[Skrive om OSL som passar til våre data ikkje berre generelt.]{.underline}
...
[Gjere dei tre neste "på nytt" med tall frå tabell over. Har teke vekk tre små tabellar som Andre meinte var overflødig.]{.underline}

#### Education

What we can find here in @tbl-inference-edu (bortevekk) is that education has its p-value on 0.79.
...
Here in @tbl-inference-demo (bortevekk) the p-value is 0.8794, which again is not possible to reject, because there is weak evidence against the null hypothesis.
