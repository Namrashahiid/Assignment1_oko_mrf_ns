...
To take a closer look at which counties in our dataset are the poorest and richest, we look at @fig-gdp_pc_nuts0. We see a common trend in forward growth in all of the countries.
...
When we take a look at some kind of noteworthy outliers between these countries, we can take a look at @fig-outlier. What we can point out here is that Ireland (IE) have some weird appearance in this graph.
...
[A log in R will transform our data in a way that will help reduce its skew. In other words it will reproduce or rescale our data more in a statistical way. (Pkt. 2 frå Andre: I do not understand what you aim to say with this.]{.underline}
...
[If we look at the R2 value for country Slovenia (SI), we can see that it is pretty high. It is the highest value, which is 1.000. The fact of the matter is that two values always fit one and other perfectly. (pkt. 1 over)]{.underline}
...
We have chosen these tree because we want to see if and how stock of vehicles, educational attainment level and population, effect growth and inequality.
[We think that a rich country have more vehicles and a higher educational attainment. We also believe that a country with a higher population have more variety in living conditions.]{.underline}
...
[Give a small interpretation of your findings!!]{.underline}

### Discuss the overall fit of your model:

[Nye tall no, men teksten er ikkje bytta ut...]{.underline}

[What has happen here is that we get many values that are 0. The reason behind this can be that we have many repeating regions (example: 16 ES11 regions). We came to a solution some days ago, but suddenly at the last minute the problem occurred.]{.underline}

[If we analyze or discuss our table above we find we can see that education, infra and life.exp has 0 as value. We see that Netherlands and Poland has same quantity of values. We are going to analyze Netherlands here, because this is country which we have looked at much in our assignment. What we are analyzing here: education, infrastructure, transport and demography.]{.underline}
...
What we can find here in @multi_linear_regression_model_1 is that education has its p-value on 0 [(Kor finne du den?)]{.underline}.
...
[Til slutt på 2, konkludere hypotesene? Kelais kan me konkludera? We think that a rich country have more vehicles and a higher educational attainment. We also believe that a country with a higher population have more variety in living conditions.]{.underline}
...
Initially we use [(kan me bruke use for detta?)]{.underline} the significance level 0.05 (5%).
...
[We aim to have a bigger $R^2$ value.]{.underline} We can conclude here by saying that the regression analysis for our all countries is somehow better than the previous analysis.
Log, $log^2$ and $log^3$ is a better functional form for our estimates on the relationship between $y_i$ and $GINIW_j$.
[However, the highest R2, which is 0.495 have we observed, when we have only estimated for the Greece data. (Vil berre at me nett ser øve i lag)]{.underline}
...
[We can see from @fig-mlr-scatterplot_fittedline that if the GDP is low and increases the quadratic estimates of degree 3, it suggest that inequality gini increases at a GDP of approximately exponential (9.1) the effect turns around the inequality starts to decrease. (Dele opp setningen/sette inn komma)]{.underline}
...
[If we look at if there is any correlation between the variables the value is 0.221.]{.underline} An interpretation which is between 0-0.30 is considered as negligible or in other words weak correlation.
...
[We will in this test use the significanse level 0,05 to test for heteroskedasticity(?).]{.underline}
...
[What we have seen so far is a interpreting regression predictevly.]{.underline} We have had many values that we have used for our inputs.
...
[And therefore ... no we can not interpret the estimates as causal. Svare på oppgåva.]{.underline}
...
[This could be one of the eventual problems that can occur.]{.underline} In our case this could potentially be Albania.
...
[To do that, we are using the penal data formel. (For å bruke dataen som me har brukt før?)]{.underline} Our aim is to use our entire dataset to estimate the effect of regional development yi on regional inequality GINIWj in a penal setup.
...
If the test score value is less than this value, then this indicates that [the test of this test]{.underline} is statistically significant.
...
Lets suppose the p-value is less than a certain significance level (5%= 0.05), [then the variable is statistically significance relationship.]{.underline}

Here in our case for the NUTS0 data the Pr t value for NUTS0 on country level is on 0.01558.
[Our 0.05 (5%)]{.underline}.
...
Now for the NUTS2 data the Pr t value for [NUTS2 on country level]{.underline} is on 0.03635.
[Our 0.05 (5%)]{.underline}.
...
-   
    ...
    -   [*`Write a consistent document that discusses the hypothesis that we can observe a significant and positive effect of development on regional inequality for your set of countries. !!!!`*]{.underline}
