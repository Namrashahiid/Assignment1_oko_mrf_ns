...
The R-spuared (R2) tell us how well the model which we are working on is fitting our data.
R2 measures the relationship between the predicted variable and response variable.
Here in this task are we looking at the relationship between regional development yi and regional inequality GINIWj.

When we look at the R2 value, we can here conclude by saying that, this R2 value 0.016 is small, and the R2 is relatively not strong, in other words weak.

Now looking at F-statistics.
...
These values that are far away from the fitted line, tell us that it does not fit the overall relationship between x-axis and y-axis (gini and GDP per capita).

### Give a verbal interpretation of your estimate coefficients:

We can see that there is a negative sign of relationship between gini and GDP per capita, because we can see that the regression coefficients explains an inverse relationship between the variables.
The linear regression line/slope is negative.
This means that one increase in the depended variable, then the dependent variable decreases.
From our scatter plot we can visualize that there is somehow a relationship between gini and GDP per capita.
