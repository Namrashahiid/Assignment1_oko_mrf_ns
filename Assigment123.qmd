...
-   *`When talking about data sources regarding monetary variables a statement about the prices
    values is based upon is essential. GDP in constant prices vs. GDP in current prices?`*
...
-   *`“Looking at Table 1 and the table descriptive statistics table over” I think you aimed to mention Table 2 here as well?`*
...
-   *`“A log in R wil transform our data in a way that will apply a logarithm to our data that
    will help reduce its skew. In other words it will reproduce or rescale our data more in a statistical way.” I do not understand what you aim to say with this.`*
...
-   *`The t-tests you present are redundant as the information is already part of the estimation
    tables you present.`*
...
-   *`“If we look at No Greece model, we can see that the p-value is: 0.059. If we now take initially the significance level 0.05 (5%). We can here conclude by saying, that we are not able to reject the null hypothesis, because the p-value is higher than the significance level:” The p-value you are interpreting here is the one related to the Ftest. Please think about what the model F test is supposed to tell us. As you stated before you have to look at the p-values of the individual estimates e.g. The stars and you can say for example that the effect of GDP on Gini is -0.002 so negative and significant at a 10% level (just one star)`*
-   *`You did not interpret the coefficients or the difference between them or the interpretation of the interaction effect model.`*
-   *`“(intercept) value, which is 0.534, we can say here that it is pretty high, which can indicate that there in a interaction between log, and in all countries.” This is wrong and just the value that the gini takes if all other variables would be zero.`*
-   *`“… value under interaction is 0.221, which is higher than the previous one…” That is in general true the new models outperform the others when looking at all data. However, the highest R2 (0.495) you have observed when only estimating using Greece data`*
-   *`“We can see from Figure 12 that the green line starts negatively, and suddenly start getting positive after 10.3 according to the x-axis.” It would be good to use the variables hear with their names. E.g. if GDP is low and increases the polynomial estimates of degree 3 suggest that inequality (gini increases) at a GDP of ruffly exp(9.1) the effect turns around inequality starts to decrease….`*
-   *`You have actually not answered the question, do you believe that the correlations you found can be interpreted as causal?`*
...
## 4.2 Alternative functional forms of our previous estimates on the relationship between $y_i$ and $GINIW_j$
...
We can see from @fig-mlr-scatterplot_fittedline that the green line starts negatively, and suddenly start getting positive after 10.3 according to the x-axis.
We can here say that there is a inverse relationship between the variables.
The green and the grey line crosses each other.
...
## 4.3 Discuss if we can interpret the estimates as causal
...
## 4.4 Potential remedies for eventual problems
