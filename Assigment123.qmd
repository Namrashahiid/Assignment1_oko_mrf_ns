...
lm.1 = lm(gini_n2*1000 ~ gdp_capita_n2, mlrdata)  %>%
  coeftest(vcov = hccm, save = TRUE)
lm.2 = lm(gini_n2*1000 ~ gdp_capita_n2 +I(edu1/100), mlrdata) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.3 = lm(gini_n2*1000 ~ gdp_capita_n2 +I(infra1/1000), mlrdata) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.4 = lm(gini_n2*1000 ~ gdp_capita_n2 + I(demo1/1000), mlrdata) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.5 = lm(gini_n2*1000 ~ gdp_capita_n2 +I(infra1/1000) + I(edu1/100) + I(demo1/1000), mlrdata) %>%
  coeftest(vcov = hccm, save = TRUE)
...
#Gen Table
  ...
  #coefs = c(Const. = "(Intercept)", GdpPerCapita = "gdp_capita_n2", Education = "I(edu1/1000)", Infrastructure = "I(infra1/1000)", Demographics = "I(demo1/1000)"),
...
```{r}
#| label: tbl-mlr-Netherland_statistics
#| tbl-cap: Multi linear regression model for Netherlands
data2010NED <- data2010 %>%
  filter(id_nuts2 != 'NL') 

data2010$NED <- ifelse(data2010$id_nuts0 == "NL", 1,0)
data2010$Oth <- ifelse(data2010$id_nuts0 == "NL", 0,1)

data2010 <- data2010
lm.10 = lm(gini_n2*1000 ~ gdp_capita_n2, filter(data2010, NED == 1))
lm.11 = lm(gini_n2*1000 ~ gdp_capita_n2, filter(data2010, NED == 0))
lm.12 = lm(gini_n2*1000 ~ gdp_capita_n2 * NED , data2010)

#Gen Table
  huxreg(
  list("Only Nederlands" = lm.10, "No Nederlands" = lm.11, "Interaction" = lm.12),
  statistics = c("N" = "nobs", 
...
In this task are we going to answer if the development is statistically different for different subsets of countries in our data.
Here we have to do a hypothesis test.
What would be sensible here to do is that we can perform a to-sided (tailed) hypothesis test:

$$
H0: B1 = 0
$$

Now when we are going to to a statistical significance, we are given a p-value.
...
When we look at the bottom line, we can see where is says Note: \*\*\* p \< 0.01; \*\* p \< 0.05; \*p \< 0.1
...
Now when we take initially the p-value for only Netherlands variable, we can see at 0.026 and compare it with the 0.05 significance level, than we can say at once, that we are rejecting the null hypothesis, because 0.026\<0.0.5

Tror jeg har skjønt dette.
Fortsetter skrivingen ila kvelden.
etter at du har comittet

## Explore alternative functional forms (e.g. log, cubical estimate, ...) of your previous estimates on the relationship between regional development yi on regional inequality GINIWj. Give an argument for two forms that might fit the data better:

To find out answer to this question/ task we have to find out the what the definition to log-estimates and cubical estimates is.

Log, logarithm can be defined as the inverse function to exponentiation.

```{r}
data2010 <- data2010
lm.13 = lm(gini_n2 ~ log(gdp_capita_n2), filter(data2010, NED == 1)) %>%
coeftest(vcov = hccm, save = TRUE)
lm.14 = lm(gini_n2 ~ log(I(gdp_capita_n2)^2), filter(data2010, NED == 1)) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.15 = lm(gini_n2 ~ log(I(gdp_capita_n2)^3), filter(data2010, NED == 1)) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.16 = lm(gini_n2 ~ gdp_capita_n2*NED, data2010) %>%
  coeftest(vcov = hccm, save = TRUE)
#Gen Table
  huxreg(
  list("Only Nederlands (log)" = lm.13, "Only Nederlands (log) ^2" = lm.14, "Only Nederlands ^3" = lm.15, "Interaction" = lm.16),
  statistics = c("N" = "nobs", 
      R2 = "r.squared", "F statistic" = "statistic",
      "P value" = "p.value"),
   stars = c("*" = 0.1, "**" = 0.05, "***" =0.01),
...
```{r}
ggplot(data = data2010, aes(x = gini_n2, y = gdp_capita_n2)) +
  geom_point() +
...
```{r}
ggplot(multi_linear_regression_model_1(data2010), aes(x = gini_n2, y = gdp_capita_n2)) +
geom_point(size = 2.5, alpha = 0.8, aes(color = NED, shape = NED)) +
geom_hline(yintercept = 0) +
geom_vline(xintercept = 0) +
geom_line(stat = "smooth", color = "orange", method = lm, alpha = 0.2, size = 1) +
geom_abline(
  intercept = lm_gdppc$coefficients[1],
  slope = lm_gdppc$coefficients[2],
...
scale_color_manual("", values = c(red_pink, "darkslategrey"), labels = c("Fra", "Male")) +
