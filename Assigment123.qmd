...
The country we re looking here is Greece.
Why we have chosen Greece as country, it is because the country have many variables, that we can use for our test.
...
```{r}
lm_function <- function(lmdata) {
ggplot(multi_linear_regression_model_1(data2010), aes(x = gini_n2, y = gdp_pc_n2)) +
...
lm.13 = lm(gini_n2 ~ log_gdppc, data = PL2010) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.14 = lm(gini_n2 ~ log_gdppc, data = ES2010) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.15 = lm(gini_n2 ~ log_gdppc, data = SI2010) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.16 = lm(gini_n2 ~ log_gdppc, data = NL2010) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.17 = lm(gini_n2 ~ log_gdppc, data = EL2010) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.18 = lm(gini_n2 ~ log_gdppc, data = data2010) %>% 
  coeftest(vcov = hccm, save = TRUE)


#Gen Table
  huxreg(
  list("Poland" = lm.13, "Spain" = lm.14, "Slovenia" = lm.15, "Netherlands" = lm.16, "Greece" = lm.17, "All countries" = lm.18 ),
  statistics = c(N = "nobs", R2 = "r.squared"),
  coefs = c(log_gdppc = "log_gdppc", const. = "(Intercept)"),
  note = "Note: {stars} T statistics in brackets."
  ) 
