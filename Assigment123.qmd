...
In the first part we are looking at data and descriptive.
In the second part we are doing cross-sectional estimates.
Continuing to the third part we will study alternative functional forms.
Finally we will be doing a penal estimate.
...
We have information on GDP per capita in multiple regions from our different countries.
...
From @tab-wealthiest and @tab-least_wealthiest we see that the wealthiest regions in our dataset are located in the Netherlands and in Norway, and that the ten poorest regions are all located in Poland.
...
To take a closer look at which counties in our dataset are the poorest and richest, we look at @fig-gdp_pc_nuts0.
We see a common trend in forward growth in all of the countries.
...
Orange line is referring to Albania´s GDP per capita.
If we compare Norway´s GDP per capita with Albania´s, there is huge difference.
In 2015 there were approximately 45 000 euro in difference.

Looking at the GDP per capita for Norway (blue line), we can see that it started quite high in around 2006, but then suddenly we can see it had a decline in 2008.
The reason behind this is most probably because of the financial crisis, which affected the society in years 2008-2009.
Suddenly in 2009 the GDP per capita started to rise again.
Continuing it had its small fall downs again around year 2015, and it raised again.
We see the same trend for the purple line (Poland), turquoise line (Netherlands) and orange line (Greece).

In the middle, the lines show the other countries.
These lines are mostly slightly rising, which means that the countries have had a steady rise in GDP per capita.

Continuing to GDP per capita in NUTS3 regions we see in @fig-gdp_pc_min the development of GDP per capita for the ten poorest regions in our dataset.
...
Poland will therefore be one of the countries we will look closer into in the next part about individual country cases.
...
We can see that the region NL113 (the orange line) shows us that it´s on the top of all other regions in Netherlands and surprisingly Norway NO071 (the pink line), even though Norway was the country with the highest GDP per capita in @fig-gdp_pc_nuts0 with NUTS0 regions (whole country).
...
In the next part about individual country cases we are also going to look closer into the Netherlands.

### Individual country cases

With this GDP per capita for specific countries, we can easily filter our information/data afterwards in the assignment.
...
Since Norway is the country with the highest GDP per capita it is interesting to study Norway closer.
When we look at the GDP per capita for Norway, in @tbl-summary_Norway, we can see that there is no difference between the values, such as Min and Max, Mean, Std.
Dev, Pctl.
The values are exactly the same.
The reason behind this is that the N value is 1.
...
The @tbl-wealthiest_NL shows us the 3 richest regions in Netherlands.
The richest region in Netherlands is NL113 (Rest of Groningen).
This region had GDP per capita on 55857.89.
The @least-wealthiest_NL shows us the poorest region in Netherlands is NL111 (East Groningen).
The GDP per capita on 17983.67.
The richest region in the Netherlands is approximately 3 times richer than the poorest.
...
We can see that there is big difference between the wealthiest regions from @tbl-wealthiest_PL and @tbl-least_wealthiest_PL (least wealthiest regions) in Poland.
...
Except from Norway, Greece are the country with the highest gdp per capita looking at @fig-gdp_pc_nuts0. We will therefore take a closer look on Greece.
Looking at @tbl-wealthiest_EL for the three wealthiest regions in Greece we see the GDP per capita for EL303 (the first region), EL306 (the second region) and EL301 (the third region).
...
Of the countries we have studied clooser, Greece has the smallest difference between the richest and poorest regions.
...
If we look at PL84 Makroregion (pink line).
It started pretty low and suddenly in year 2012-2013 it rised pretty high.
Most likely it is because of some NA values that are reported as 0 form 2000 to 2012.
Then the correct values are reported and it looks like the level of inequality suddenly have a big jump.
The other lines are somehow the same.

#### Gini for Greece

In @fig-gini_greece we can see level of inequality over time for regions in Greece.
...
This region has a different curve than many of the other regions in Greece.

## 1.5 Outlier discussion

In this next part we are going to discuss outliers.
To start we are defining the word outlier.
The definition of this word is when a value or an observation is distant from other observations, that is to say, a data point that differs significantly from other data points.

### Discuss briefly if there are noteworthy outliers:

When we take a look at some kind of noteworthy outliers between these countries, we can take a look at @fig-outlier.
What we can point out here is that Ireland (IE) have some weird appearance in this graph.
We can see that some outliers emerge for Ireland.
It start with a point, than have a big gap before the gini starts raising again.
Then the gap continues and suddenly gini is back.
...
This is weird.
A reason for this can be that regions with NA values get 0 instead of NA, as mentioned earlier.
It could therefore make sense to exclude these observations in later studies.

As we have seen under individual country cases, we can see that Poland (PL) have very rich regions and at the same time very poor regions as well.
As we can see in @fig-outlier, there is no clear outliers for Poland (PL), because there is a long line with with small gaps.
...
The first task in this assignment is to estimate the effect of regional development $y_j$ on regional inequality $GINIW_j$ for the year 2010.
To study growth and inequity, it is essential for us to look at GDP per capita.
...
An important fact to come up with is that, although GDP per capita is often used as a broad measure of average living standards, high GDP per capita does not necessarily mean high level of households disposable income, a key measure of average material capita in the OECD.
...
Log is a method which transforms the data, in not a very easy way, but it creates the best log transformed data in a statistical way.
The log can be defined as an built in function in R, which is used to compute the logarithm of the specific value.
...
As we have mentioned earlier, the gini value is from 0 to 1.
...
One important thing here to point out is that the reason behind that the values are equal to zero can be because there are some data/values that are lost or not there for this specific year which we have chosen (NA).

Further down in @tbl-stastics_log_gdppc, we find N values for six of our countries.
These are Spain (ES), Netherlands (NL), Norway (NO), Poland (PL), Slovenia (SL) and Greece (EL).
What we can see is that Norway (NO) only have 1 observation.
That is because in our dataset it is only one NUTS3 region that is related to Norway (NO), as mentioned earlier.
On the other side we see Greece (EL) have 13 obervation.
...
What we are going to do in this part is to describe the effect of regional development on regional inequality, and then we are going to analyze this by the help of regression analysis.
...
First we are going to study our selected countries for the year 2010.
Then we are going to use simple regression model to look at the result of regional development inequality.
...
Regression analysis is a process which is used to see the relationship between different dataset variables.
This data analysis method is used to see the relationship between independent and depended variables.
...
The fact of the matter is that a line always fits perfect between two values.
When there is two observations they always fit one another perfectly; that is called a statistical artefact.
...
### **Plot the relastionship between regional developement** $y_i$ **and regional inequality** $GINIW_j$**, and the fitted line corresponding to your estimate**

In this part are we going to analyze estimation for our five countries.
The countries we are going to use as a estimation for our plots are: Spain (ES), Poland (PL), Slovenia (SI), Greece (EL) and Netherlands (NL).

Coefficients of resolution is important for our next steps in this assignment.
So the first we are going to do is to analyze all five countries in one figure.

```{r}
#| label: fig-mlr-scatterplot_fitted
#| fig-cap: Scatterpolt with fitted line corresponding to estimate
...
@fig-mlr-scatterplot_fitted shows us that when prosperity increases, income and GDP per capita are less different.
...
On this graph @fig-mlr-scatterplot_fitted we can see Log GDP per capita on the x-axis and Gini NUTS2 values at the y-axis.
...
The dots indicates the observations for the regions in those five countries mentioned above.
...
What we can see from @fig-lm-res_pred is that the observations are pretty spread.
...
To plot predicted values against residuals is one way to visually check for heteroskedasticity.
We will discuss this closer in the next part.

### Classical assumptions of OSL

First of all a definition to this classical OLS assumption: Ordinary Least Squares (OLS) linear regression is a statistical technique for analyzing and designing linear relationship between a response and for one and more variables.
If the relationship between variables are linear, then a straight line can be fit to the specific data.
...
**Linearity:** As long as the model which
we are focusing on satisfies the OLS assumptions for linear regression, the
estimates will be good.
Here the population relationship is linear with respect
to the parameter with an additional error term.
This seems a bit fishy when
...
There is a variation in X vs. Y.
If we look at the figure of GDP per capita ​​this seems fine.

**Random Sampling:** Random sample is a part of the sampling technique.
We have a random sample of our model.
We draw a random sample from the population of interest.
It's not obvious that we can choose data availability.
However, this seems to be a minor problem.

**Exogeneity:** The X variable is exogenous (i.e. E (u\|X = 0).
This is very questionable, as there are many potential correlations between inequality and development, variables can be excluded, and we even have legitimate concerns about the problem of reverse causation.

**Homoskedasticity:** Here we look at the error term, which has the same variation for each value of the independent variable (i.e. Var(u\|X = o2).
...
Looking at the plot of residuals versus predicted values, @fig-lm-res_pred, we must question the constant variance of the residuals and thus test for heteroscedasticity.
...
**Normality:** The population error term is normally distributed with mean zero and variance (i.e. u \~ N(0, $σ^2$)).
The details are large enough that we can assume asymptotic normality for OSL.
...
These three datasets are related to transport infrastructure, education and demographics.

**Transport infrastructure**

The file 'tran_r\_vehst.csv' shows us stock of vehicles by category and NUTS2 regions.
Our dataset contains all vehicles except trailers and motorcycles.
...
The data is annually calculated of EU Labour Force Survey data.

**Demographics**

Finally the 'demo_r\_pjangroup.csv' shows us the population on 1st January by age group, sex and NUTS 2 region.
This dataset shows us population and social conditions.
It contains information of usually resident population and represents the number of inhabitants of a given area on 1st of January of the year.

We have chosen these three because we want to see if and how stock of vehicles, educational attainment level and population, effect growth and inequality.
We think that a rich country have more vehicles and a higher educational attainment.
We also believe that a country with a higher population have more variety in living conditions.
...
demo <- read_csv('demo_r_pjangroup.csv') %>%
  ...
  mutate(population_1jan = OBS_VALUE, year = TIME_PERIOD, id_nuts2 = geo) %>%
  select(id_nuts2, year, population_1jan)
...
filter(TIME_PERIOD == 2010 & age == "Y25-64" & sex == "TRUE" & isced11 == "ED0-2" & unit == "PC") %>%
  ...
  select(unit, geo, age, sex, TIME_PERIOD, OBS_VALUE) %>%
  mutate(edu_level = OBS_VALUE, year = TIME_PERIOD, id_nuts2 = geo) %>%
  select(id_nuts2, year, edu_level)
...
infra <- read_csv('tran_r_vehst.csv') %>%
  ...
  mutate(vehicles = OBS_VALUE, year = TIME_PERIOD, id_nuts2 = geo) %>%
  select(id_nuts2, year, vehicles)
```
...
We are now moving on to multiple regression model.
The question is what is multiple regression model?
...
lm.2 = lm(gini_n2*1000 ~ gdp_pc_n2 +I(edu_level/100), mlrdata) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.3 = lm(gini_n2*1000 ~ gdp_pc_n2 +I(vehicles/1000), mlrdata) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.4 = lm(gini_n2*1000 ~ gdp_pc_n2 + I(population_1jan/1000), mlrdata) %>%
  coeftest(vcov = hccm, save = TRUE)
lm.5 = lm(gini_n2*1000 ~ gdp_pc_n2 +I(vehicles/1000) + I(edu_level/100) + I(population_1jan/1000), mlrdata) %>%
  coeftest(vcov = hccm, save = TRUE)
...
#Gen Table
  ...
  statistics = c(N = "nobs", R2 = "r.squared"),
  #coefs = c(education = "edu_level", transportInfrastructure = "vehicles", demographics = "population_1jan", const. = "(Intercept)"),
  #coefs = c(Const. = "(Intercept)", GdpPerCapita = "gdp_pc_n2", Education = "I(edu_level/1000)", Infrastructure = "I(vehicles/1000)", Demographics = "I(population_1jan/1000)"),
...
We are now giving a small interpretation of our regression model:

In this regression model the first model (1) has 45 observations, with R2 value 0.060.
R2 shows us if our dependent variable has an effect or not.
This R2 value for model 1, is not so high.
This tells us that our dependent variable has not so much effect.
Model 2, 3, 4 and 5 has 32 observations in total.
But the R2 values are different.
If R2 value increases for each new model, than we have an connection.
We can conclude by saying that the models does not nececarily have an connection because it starts with and connection between model 1 and 2, than suddenly it dissapear from model 3 and 4.
The connection continues between model 4 and 5.

### Discuss the overall fit of your model:

If we discuss the overall fit of our model now, we see that our model does not have constant R2 values.
We are now using a 0.05 (5%) significance level.
Our R2 values are higher than the significance level of 0.05.
This indicates that there is an significant correlation of the determination.

### Discuss the inference related to your findings

For this last part we are looking at the same five counties as before.
We will now test and analyze if the variables has an effect upon growth and inequality.
...
What we can find in @tbl-mlr-data2010 is that education has its p-value on 0.
Now when the p-value is a probability, than it can not exceed 1.
The p-value has to be between 0-1.
In our case we find a value on: 52.442.
Therefore we are calling our p-value 0 in this case.
...
When we look at the p-value for the infrastructure, we find that the p-value is 0.011.
...
In @tbl-mlr-data2010 the p-value for demographics is 0.007, which again is possible to reject, because there is strong evidence against the null hypothesis.

We can conclude by saying, that we don't see a connection between our variables: vehicles, education attainment level and population.

# 3 Assignment 3

In this third assignment are we going to focus on alternative function forms.
First we are going to test if the effect of development is significantly different for different subset of countries in our dataset.
We are using the same data that we used in assignment 1 and the first part of assignment 2.
...
Here in this task are we going to check the effect of the development.
...
Initially we use the significance level 0.05 (5%).
...
F-test is used whether the variances of two populations (A and B) are equal.
Here in this case if we look at the model over and take the effect of GDP on Gini -0.002, it is negative and significant at a 10% level (with only one star).

Next we are looking at the Only Greece model.
The p-value for Only Greece is 0.007.
If we compare 0.007 with a significance level of 0.05 (5%), we then immediately know that we have to reject the null hypothesis, because the p-value is smaller that the significance value/level.
...
When we take a look at the interaction model.
This interaction model tells us, or in other words allow us, to find the effect between No Greece model and Only Greece model.

If we look at intercept p-value on 0.063 and use 0.05 (5%) significance level, we can conclude by saying that we can not reject the null hypothesis here because the p-value is larger than the significance level on 0.05.
...
To find the answer to this task we have to find the definition to log estimates and cubical estimates.
Log, logarithm, can be defined as the inverse function to exponentiation.

Here in this part are we are we going to look at all of our countries.
Now the question is what does $log^2$ tell us.
$Log^2$ is a mathematical function that calculates the binary logarithm.

Now looking at @tbl-mlr-all_counties the p-value for interaction is: 0.016.
If we take 0.05 (5%) as significance level, we can see that the p-value is smaller than the confidence level 0.05.
...
We can see that totally there is 45 observations (N) for all countries.
...
In this part are we looking at the relationship between regional development $y_i$ and regional inequality $GINIW_j$.
...
We see in the first table that the interaction value for gdp_pc_n2:GRC is 0,009.
...
We aim to have a bigger $R^2$ value.
...
The highest $R^2$ value which we have observed is 0.495.
...
Now when we have more then two varaiables, we want to find the correlation between the one variable and all others.
...
When these lines crosses each other, then this hardly indicates that there is a correlation between the polynomial regression at 2nd and 3rd degree.
...
Here in this case we are talking about the observation (dot), which is on the top on the right side GDP per capita (log) 10.7 and approximately on 0.18 on gini.
This observations is a bit far away from the fitted observations.
...
In comparison with line in @fig-mlr-scatterplot_fitted, we can see that these lines fit our data better.

### Verbal interpretation of our estimate coefficients

We can see from @fig-mlr-scatterplot_fittedline that GDP is low and increases the quadratic estimates of degree 3.
This suggest that inequality gini increases at a GDP of approximately exponential (9.1).
The effect turns around and the inequality starts to decrease.
...
If we compare linear regression with polynomial regression, then we can say that the effect is varying when it comes to polynomial regression.
...
To look at if there is any correlation between the variables, we have to use the value 0.221.
An interpretation which is between 0-0.30 is considered as negligible or in other words weak correlation.
So we can say that this 0.221 value is a weak correlation between the interpreted values we have found.
...
We will in this test use the significanse level 0,05 to test for heteroskedasticity.
...
Here we have got a p-value after we have used the Breusch-Pagan test.
...
Our p-value here is also high with 0.8285633.
...
First of all, causal interpretation rests on regression analysis.
...
Causal inference can be defined as a special case, where the goal is to predict if we could choose different options.
...
The tables and figures from our previous tasks have allowed us to see different values of several inputs.
...
And therefore we can conclude by saying that we can not interpret the estimates as causal.
...
First it is important to come up with that the EU´s economic and fiscal rules promote economic stability and growth.
...
We are starting this assignment by using the same data as we used from assignment 1.
Our aim is to use our entire dataset to estimate the effect of regional development yi on regional inequality GINIWj in a penal setup.
...
#Gen Table
  huxreg(
  list( "NUTS0 FE" = plm.1, "NUTS2 FE" = plm.2, "year FE" = plm.3, "NUTS2 & year FE" = plm.4 , "NUTS0 & year FE" = plm.5 ),
  statistics = c(N = "nobs", R2 = "r.squared"),
  stars = c("*" = 0.1, "**" = 0.05, "***" = 0.01),
  note = "Note): {stars} T statistics in brackets."
  )
...
Now we are going to look at the output values from the panel estimation code.
...
First the estimation value is very low at -29457,40 which indicates that the country fixed effect decreases by -29457,40.

Continuing the standard-error value is on 12153.32.
We can see that the std-error value is pretty high, which tells us that the values are more spread out and are around the mean in the dataset. 

The T-value here is: -2.4238.
If the test score value is less than this value, then this indicates that the test of this observation is statistically significant.
...
The estimation is lower than the previous estimation.
Here the value is -32378.81.
This shows us how weak the estimation is and it indicates that the effect decreases by -32378.81. 
 

The standard-error value is on 16700.21.
We can see that the std-error value is pretty high.
This tells us that the values are spread out and are around the mean in a dataset.

Finally the t-value is: -1.9388.
...
In this case our statistic significance value is lower than the statistic significance level at 5% divided in two 2,5% (0.025).
That's why our result is not statistical significance. 
 

**NUTS2 fixed effects**

The third panel estimation shows us the NUTS2 fixed effect data.
The estimation value is -22167.71.
This is pretty low, which indicates that the NUTS2 effect decreases by -22167.71.

Second the standard-error is on 10578.32.
This value is high, not as high as in the previous ones, but this again indicates that our values are more spread out and are more around the mean in the NUTS2 fixed effects. 

The T-value is -2.0956.
Again, if the test value score is less than this value, then this indicates that the test of this model is statistically significance.
...
Lets suppose the p-value is less than a certain significance level (5%= 0.05), then the variable is statistically significance in relationship.
...
Now for the NUTS2 data the Pr t value for NUTS2 on regional level is on 0.03635.
We can then say that the p-value is less than the significance level on 0.05.
This indicates that the variable is statistically significant.

**NUTS0 & year fixed effects**
...
Than we can conclude by saying that this indicates that the test of this model is is statistically significance.
This is because our T-value is lower than the statistic significance level at 5%, devided into 2= 2.5%= 0.025.
We can conclude by saying that our result here is statistically significance.

## Conclusion

In this paper we have looked at data and descriptive, cross-sectional estimates, alternative functional forms and penal estimates.
In assignment 1 we have seen that both GDP-per capita and gini had a stable growth in all of our selected countries.
We have seen that the financial crisis (2008-2009) had an big effect upon many of our countries, but that the countries also quickly recovered.

In assignment 2 we have looked at relationship between regional development and regional inequality.
We alo looked at further determinants of regional inequality, by using new data of transport infrastructure, education and demographics.
We did not found any effect between these new variables and regional development and inequality.

Assignment 3 has shown us, alternative functional forms for our dataset.
We found out that polynomial function on 2nd and 3rd degree fitted our dataset better than linear regression.

Further in assignment 4 we used our entire dataset to estimate the regional development and inequality in a penal dataset.
What we have found out in this assignment is different observations, when it comes to values: standard-error, T-value and penal estimation.
The values which we got from the table, showed us how much effect it had upon our observations.

Finally, we would like to conclude by saying that: We have learned use of statistical methods to develop theories.
We have also tested hypothesis in economics.
...
#| tbl-cap: Least wealthies regions
tmp %>%
  filter(year == 2010) %>%
  select(GDP_capita, id_nuts2, id_nuts0) %>%
